...
{
    public partial class editweldingpiece : Form
    {
        ...
        bool infohitung = false;

        public editweldingpiece()
        {
            InitializeComponent();
        }
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT bstok FROM Rb_Stok ORDER BY id_stok DESC OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY;", conn);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                lblstokdatasebelum.Text = reader["bstok"].ToString();
            }
            else
            {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("SELECT bstok,wpe1,wpe2,wbe1,wbe2,wastekg FROM Rb_Stok ORDER BY id_stok DESC OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY;", conn);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
        ...
        private void setdefault()
        {
            ...
            txtkeluar.Clear();
            lblstokupdate.Text = "-";
            lblstokdatasebelum.Text = "-";
            lblstokdatasekarang.Text = "-";
            txtpbar.Clear();
            txtsbarmm.Clear();
            sawinge1.Clear();
            sawinge2.Clear();
        ...
        private void settrue()
        {
            ...
            txtpbar.Enabled = true;
            txtsbarmm.Enabled = true;
            sawinge1.Enabled = true;
            sawinge2.Enabled = true;
            lathee1.Enabled = true;
            lathee2.Enabled = true;
            pkeluare1.Enabled = true;
            pkeluare2.Enabled = true;
            txtsbarkg.Enabled = true;
        ...
        private void setfalse()
        {
            ...
            txtpbar.Enabled = false;
            txtsbarmm.Enabled = false;
            sawinge1.Enabled = false;
            sawinge2.Enabled = false;
            lathee1.Enabled = false;
            lathee2.Enabled = false;
            pkeluare1.Enabled = false;
            pkeluare2.Enabled = false;
            txtsbarkg.Enabled = false;
        ...
        private void hitungrb()
        {
            ...
            lblstokupdate.Text = totalrb.ToString();
        }
        ...
        private void editweldingpiece_Load(object sender, EventArgs e)
        {
            tampil();
            datecari.Checked = false;
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                noprimary = Convert.ToInt32(row.Cells["id_stok"].Value);
                ...
                lblstokdatasekarang.Text = row.Cells["bstok"].Value.ToString();
        ...
        private void btnsimpan_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("UPDATE Rb_Stok SET bmasuk = @bmasuk, bkeluar = @bkeluar, bstok = @bstok, bsisamm = @bsisamm, bpe1 = @bpe1, bpe2 = @bpe2, bbe1 = @bbe1," +
                        "bbe2 = @bbe2, rbkeluare1 = @rbkeluare1, rbkeluare2 = @rbkeluare2, wpe1 = @wpe1, wpe2 = @wpe2, wbe2 = @wbe2, bsisakg = @bsisakg, wastekg = @wastekg, e1mm = @e1mm, e2mm = @e2mm," +
                    ...
                    cmd.Parameters.AddWithValue("@bstok", lblstokupdate.Text);
